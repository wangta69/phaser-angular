...
export class playGame extends Phaser.Scene {
        ...
        for(let i = 1; i < simpleSlope.length; i++){
            ...
            if(Phaser.Math.Between(0, 100) < this.gameOptions.rocksRatio && (mountainStart.x > 0 || i != 1)){
                ...
                if(this.rocksPool.length == 0){
                    ...
                    let rock: any = this.matter.add.circle(rockX, rockY, size, {
                        isStatic: true,
                        angle: angle,
                        friction: 1,
                        restitution: 0,
    ...
    private addCar(posX: number, posY: number){
        ...
        let floor = (Phaser.Physics.Matter as any).Matter.Bodies.rectangle(posX, posY, 100, 10, {
            label: 'car'
        });
        let rightBarrier = (Phaser.Physics.Matter as any).Matter.Bodies.rectangle(posX + 45, posY - 15, 10, 20, {
            label: 'car'
        });
        let leftBarrier = (Phaser.Physics.Matter as any).Matter.Bodies.rectangle(posX - 45, posY - 15, 10, 20, {
            label: 'car'
        });
